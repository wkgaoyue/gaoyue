...
# 浏览商品信息
def goodsindex(request,gIndex=1):
    # 执行数据查询，并放置到模板中
    ...
    if request.GET.get('goods','') != '':
        ...
        who.append('goods='+request.GET.get('goods'))
    ...
    if gIndex == '':
        gIndex = '1'
    gIndex = int(gIndex) #类型转换int
    list2 = g.page(gIndex) #获取当前页数据
    glist = g.page_range #获取页码信息
    #封装分页信息
    context = {'glist': glist, 'gIndex': gIndex, 'who':who,"goodslist":list2}
    return render(request,"myadmin/goods/index.html",context)
